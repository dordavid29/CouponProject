...
public class PurchasesDao implements IPurchasesDao {
	...
	public void updatePurchase(Purchase purchase) throws ApplicationException {
		...
		try {
			...
			e.printStackTrace();
			throw new ApplicationException(e, ErrorTypes.UPDATE_ERROR, " Update purchase has failed");
		} finally {
			JdbcUtils.closeResources(connection, preparedStatement);
		}
	}
	...
	public void deleteExpiredPurchases() throws ApplicationException {
		...
		try {
			...
			connection = JdbcUtils.getConnection();
			String sqlStatement = "DELETE FROM purchases WHERE coupon_id IN (SELECT coupon_id FROM coupons WHERE end_date < curdate())";
			preparedStatement = connection.prepareStatement(sqlStatement);
			preparedStatement.executeUpdate();
