...
public class CouponsDao implements ICouponsDao {
	...
	public List<Coupon> getCustomerCoupons(long customerId) throws ApplicationException {
		...
		try {
			connection = JdbcUtils.getConnection();
			String sqlStatement = "SELECT * FROM coupons WHERE coupon_id IN (SELECT coupon_id FROM purchases WHERE customer_id=?)";
			preparedStatement = connection.prepareStatement(sqlStatement);
	...
	public List<Coupon> getCustomerCouponsByCategory(long customerId, Category category) throws ApplicationException {
		...
		try {
			connection = JdbcUtils.getConnection();
			String sqlStatement = "SELECT * FROM coupons WHERE category=? AND coupon_id IN (SELECT coupon_id FROM purchases WHERE customer_id=?)";
			preparedStatement = connection.prepareStatement(sqlStatement);
			preparedStatement.setLong(1, customerId);
	...
	public List<Coupon> getCustomerCouponsByMaxPrice(long customerId, double maxPrice) throws ApplicationException {
			...
			try {
				connection = JdbcUtils.getConnection();
				String sqlStatement = "SELECT * FROM coupons WHERE coupon_price<? AND coupon_id IN (SELECT coupon_id FROM purchases WHERE customer_id=?)";
				preparedStatement = connection.prepareStatement(sqlStatement);
				preparedStatement.setLong(1, customerId);
